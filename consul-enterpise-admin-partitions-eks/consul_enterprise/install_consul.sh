...
if [ "${CONSUL_DEPLOY_TYPE}" = "server" ]; then
  ...
  yq e ".client.join = [\"${PartitionIp}\"]" -i /Users/webdog/github/consul-k8s-admin-partitions/consul_enterprise/consul-values-"${CONSUL_DEPLOY_TYPE}".yaml
  yq e ".externalServers.hosts = [\"${PartitionIp}\"]" -i ./consul_enterprise/consul-values-"${CONSUL_DEPLOY_TYPE}".yaml
